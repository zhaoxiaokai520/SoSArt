...
public class AB_GatherResInfo
{
	...
	public static string GenerateABMD5()
	{
		...
		{
			text += AB_AssetBuildMgr.mManifest.GetAssetBundleHash(allAssetBundles[i]);
		}
	...
	private static void MakeManifest2PackerDependency()
	{
		...
		foreach (string current in AB_GatherResInfo.mResPackerInfoSet.m_assetGroupInfosAll.Keys)
		{
			...
			string[] allDependencies = AB_AssetBuildMgr.mManifest.GetAllDependencies(text);
	...
	public static void GatherCollectRedundancy(List<AB_AssetBuildMgr.CollectRedundancy> ll)
	{
		foreach (AB_AssetBuildMgr.CollectRedundancy current in ll)
		{
