...
public class AB_HeroBattleBuild : AB_HeroBuildBase
{
	...
	private void CreatePacketBuild()
	{
		...
		for (int i = 0; i < directories.Length; i++)
		{
			...
			{
				this.mHeroBattleBase = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, text + "_battle", 0);
			...
			else if (directoryInfo.Name.ToLower().Equals("action") && this.mHeroBattleBase != null)
			{
				int num = 0;
				num |= 8;
				this.mHeroAction = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_ACTION, text + "_skill", num);
				this.mHeroAction.AddCmd(new AB_HeroCmdAction(directoryInfo));
			}
			else if (directoryInfo.Name.Contains(text))
			{
				AB_HeroPacketBuild aB_HeroPacketBuild = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, directoryInfo.Name + "_battle", 0);
			...
			{
				AB_HeroBuildMgr.mActorInfo.AddCmd(new AB_HeroCmdActorInfo(directoryInfo));
			}
			else if (directoryInfo.Name.ToLower().Equals("weapon"))
			{
				...
				for (int j = 0; j < directories2.Length; j++)
				{
					...
					AB_HeroPacketBuild aB_HeroPacketBuild2 = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, text + "_" + directoryInfo4.Name + "_battle", 0);
			...
			else if (directoryInfo.Name.ToLower().Equals("gem"))
			{
				...
				for (int j = 0; j < directories2.Length; j++)
				{
					...
					AB_HeroPacketBuild aB_HeroPacketBuild3 = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, text + "_gem_" + directoryInfo5.Name, 0);
			...
			{
				this.mHeroBattleEffectBase = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, text + "_battle_effect", 0);
