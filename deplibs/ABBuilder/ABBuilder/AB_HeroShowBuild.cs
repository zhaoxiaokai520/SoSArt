...
public class AB_HeroShowBuild : AB_HeroBuildBase
{
	...
	private void CreatePacketBuild()
	{
		...
		for (int i = 0; i < directories.Length; i++)
		{
			...
			{
				this.mHeroBattleBase = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, text + "_show", 0);
			...
			{
				AB_HeroPacketBuild aB_HeroPacketBuild = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, directoryInfo.Name + "_show", 0);
			...
			else if (directoryInfo.Name.ToLower().Equals("weapon"))
			{
				...
				for (int j = 0; j < directories2.Length; j++)
				{
					...
					AB_HeroPacketBuild aB_HeroPacketBuild2 = new AB_HeroPacketBuild(AB_AssetBuildMgr.E_ABBUNLDE_TYPE.E_CHARACTER, text + "_" + directoryInfo4.Name + "_show", 0);
