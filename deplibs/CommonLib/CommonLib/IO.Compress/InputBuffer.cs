	...
	internal class InputBuffer
	{
		...
		public bool EnsureBitsAvailable(int count)
		{
			if (this.bitsInBuffer < count)
			{
				...
				int num = this.start;
				this.start = num + 1;
				this.bitBuffer = System.Convert.ToUInt32((arg_3F_0 | arg_34_0[num] << (this.bitsInBuffer & 31)));
				this.bitsInBuffer += 8;
				if (this.bitsInBuffer < count)
				{
					if (this.NeedsInput())
					...
					num = this.start;
					this.start = num + 1;
					this.bitBuffer = System.Convert.ToUInt32(arg_8F_0 | arg_84_0[num] << (this.bitsInBuffer & 31));
					this.bitsInBuffer += 8;
				}
			}
			return true;
		...
		public uint TryLoad16Bits()
		{
			if (this.bitsInBuffer < 8)
			{
				if (this.start < this.end)
				{
					...
					int num = this.start;
					this.start = num + 1;
					this.bitBuffer = System.Convert.ToUInt32(arg_43_0 | arg_38_0[num] << (this.bitsInBuffer & 31));
					this.bitsInBuffer += 8;
				}
				if (this.start < this.end)
				{
					...
					int num = this.start;
					this.start = num + 1;
					this.bitBuffer = System.Convert.ToUInt32(arg_91_0 | arg_86_0[num] << (this.bitsInBuffer & 31));
					this.bitsInBuffer += 8;
				}
			}
			else if (this.bitsInBuffer < 16 && this.start < this.end)
			{
				...
				int num = this.start;
				this.start = num + 1;
				this.bitBuffer = System.Convert.ToUInt32(arg_E8_0 | arg_DD_0[num] << (this.bitsInBuffer & 31));
				this.bitsInBuffer += 8;
			}
