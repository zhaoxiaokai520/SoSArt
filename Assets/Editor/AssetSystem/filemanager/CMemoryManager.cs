...
public class CMemoryManager
{
	...
	{
		uint lowPart = (uint)CMemoryManager.ReadInt(data, ref offset);
		uint highPart = (uint)CMemoryManager.ReadInt(data, ref offset);
	...
	{
		int size = CMemoryManager.ReadShort(data, ref offset);
        string @string = Encoding.UTF8.GetString(data, offset, size);
        offset += size;
		return @string;
	}
	...
	public static DateTime ReadDateTime(byte[] data, ref int offset)
	{
		...
		if (val < maxValue.Ticks)
		{
			DateTime minValue = DateTime.MinValue;
			if (val > minValue.Ticks)
			{
				DateTime result = new DateTime(val);
				return result;
			}
		}
